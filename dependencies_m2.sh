...
sudo su
cp /vagrant/backup_m2.sh /bin/backup.sh
chmod +x backup.sh
echo "0 * * * * bin/backup.sh" >> /var/spool/cron/crontabs/root
...
docker run --name db_container -e POSTGRES_USER=pguser -e POSTGRES_PASSWORD=parola -p 192.168.56.111:5432:5432 -v /pg/pg_dataset:/var/lib/postgresql/data -d postgres
